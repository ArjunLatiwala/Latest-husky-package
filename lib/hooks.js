...
if [ -n "$LINT_FILES" ]; then
  ...
  if [ -n "$ESLINT_BIN" ]; then
    ...
    # Function to run cs-setup check-hooks safely
    run_check_hooks() {
      if [ -f "./node_modules/.bin/cs-setup" ]; then
        ./node_modules/.bin/cs-setup check-hooks
      elif [ -f "$(npm root)/cs-setup/bin/index.js" ]; then
        node "$(npm root)/cs-setup/bin/index.js" check-hooks
      else
        npx --no-install cs-setup check-hooks || true
      fi
    }

    if [ $HAS_CONFIG -eq 0 ]; then
      echo "[ESLint] No configuration found. Attempting automatic restoration..."
      run_check_hooks
      # Re-check
      if [ -f "eslint.config.js" ] || [ -f "eslint.config.mjs" ] || [ -f "eslint.config.cjs" ] || \
         [ -f ".eslintrc.js" ] || [ -f ".eslintrc.cjs" ] || [ -f ".eslintrc.yaml" ] || \
    ...
    if [ $HAS_CONFIG -eq 1 ]; then
      ...
      if [ $LINT_EXIT -ne 0 ]; then
        echo ""
        echo "[ESLint] ✖ Linting detected issues (exit code $LINT_EXIT). Continuing commit (non-blocking)..."
      else
        echo "[ESLint] Lint check passed. ✔"
      fi
    else
      echo "[ESLint] No configuration found (eslint.config.js, .eslintrc.*, or package.json) — skipping."
      echo "[ESLint] Tip: Create an ESLint config file or add 'eslintConfig' to package.json."
    fi
  else
