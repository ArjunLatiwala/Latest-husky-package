...
# Detect if this is a CLI/utility package vs a server package
detect_project_category() {
    ...
    if grep -q -E '"(express|fastify|koa|hapi|sails|loopback)"' package.json 2>/dev/null; then
        echo "server"
        return
    fi
    
    # Check for frontend frameworks
    if grep -q -E '"(react|vue|angular|next|nuxt|svelte)"' package.json 2>/dev/null; then
        echo "frontend"
        return
    fi
    
    # Default to server if start scripts exist
    if npm run 2>/dev/null | grep -E "^\s*(start|dev|serve)" >/dev/null; then
        echo "server"
    else
        echo "utility"
    fi
...
run_smoke_tests() {
    ...
    local project_category=$(detect_project_category)
    log_info "Project category detected: $project_category"
    
    # Skip smoke tests for CLI/utility packages
    case "$project_category" in
        "cli")
            log_info "CLI package detected - skipping smoke tests (no server to start)"
            return 0
            ;;
        "utility")
            log_info "Utility package detected - skipping smoke tests (no server to start)"
            return 0
            ;;
    esac
